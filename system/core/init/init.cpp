...
void service_start(struct service *svc, const char *dynamic_args)
{
    ...
    svc->time_started = 0;
/* lenge add for car reverse detete uvc camera at 20180227
    if(((!strncmp(svc->name,"healthd",7))||(!strncmp(svc->name,"bootanim",8))||(!strncmp(svc->name,"zygote",6))||(!strncmp(svc->name,"surfaceflinger",14)))&&((!strcmp(bootmode, "charger")))){
         ERROR("stopping '%s'\n", svc->name);
